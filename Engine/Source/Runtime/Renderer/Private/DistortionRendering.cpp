...
bool FSceneRenderer::RenderDistortionAccumulation(FRHICommandListImmediate& RHICmdList, TRefCountPtr<IPooledRenderTarget> DistortionRT, FTexture2DRHIRef SceneDepth, bool bClearStencil)
{
    ...
    if(DistortionRT)
    {
		...
		if (!SceneDepth) { SceneDepth = SceneContext.GetSceneDepthSurface(); }
        ...
        FRHIDepthRenderTargetView DepthView( SceneDepth, ERenderTargetLoadAction::ELoad, ERenderTargetStoreAction::ENoAction, FExclusiveDepthStencil::DepthRead_StencilWrite );
        FRHISetRenderTargetsInfo Info( 1, &ColorView, DepthView );		
		Info.SetClearDepthStencil(false, bClearStencil);
...
void FClusteredForwardShadingSceneRenderer::RenderForwardDistortion(FRHICommandListImmediate& RHICmdList)
{
	...
	bool bDirty = false;
	{
		// Create a texture to store the resolved light attenuation values, and a render-targetable surface to hold the unresolved light attenuation values.
		{
			FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(SceneContext.GetBufferSizeXY(), PF_B8G8R8A8, FClearValueBinding(FLinearColor(0,0,0,0)), TexCreate_None, TexCreate_RenderTargetable, false));
			Desc.Flags |= TexCreate_FastVRAM;
			GRenderTargetPool.FindFreeElement(RHICmdList, Desc, DistortionRT, TEXT("Distortion"));
