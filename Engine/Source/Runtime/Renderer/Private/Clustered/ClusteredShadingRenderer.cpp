 ...
 */
bool FClusteredForwardShadingSceneRenderer::InitViews(FRHICommandListImmediate& RHICmdList, struct FILCUpdatePrimTaskData& ILCTaskData, FGraphEventArray& SortEvents)
{
...
void FClusteredForwardShadingSceneRenderer::InitViewsPossiblyAfterPrepass(FRHICommandListImmediate& RHICmdList, struct FILCUpdatePrimTaskData& ILCTaskData, FGraphEventArray& SortEvents)
{
...
void FClusteredForwardShadingSceneRenderer::Render(FRHICommandListImmediate& RHICmdList)
{
	...
	{
		SCOPED_DRAW_EVENT(RHICmdList, Foreground);
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		for (TSparseArray<FLightSceneInfoCompact>::TConstIterator LightIt(Scene->Lights); LightIt; ++LightIt)
		{
			...
			bool const bSupportedType =
				...
				|| (CLUSTERED_SUPPORT_SPOT_LIGHTS && LightType == LightType_Spot)
				// We want to support LightType=Directional only in the editor.  We do this so we can use preview views
				// but disable the code in the shader for the game to save shader cost (see ClusteredShadingCommon.usf)
