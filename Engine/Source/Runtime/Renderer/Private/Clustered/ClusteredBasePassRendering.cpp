...
IMPLEMENT_CLUSTERED_SHADING_BASEPASS_LIGHTMAPPED_SHADER_TYPE(TUniformLightMapPolicy<LMP_SHINDVL>, FCachedVolumeIndirectLightingPolicy );
IMPLEMENT_CLUSTERED_SHADING_BASEPASS_LIGHTMAPPED_SHADER_TYPE(TUniformLightMapPolicy<LMP_MDL_SHINDVL>, FSimpleDirectionalLightAndSHDirectionalVolumeIndirectPolicy);
IMPLEMENT_CLUSTERED_SHADING_BASEPASS_LIGHTMAPPED_SHADER_TYPE(TUniformLightMapPolicy<LMP_MDL_CSM_SHINDVL>, FSimpleDirectionalLightAndSHDirectionalCSMVolumeIndirectPolicy );
#endif
...
public:
	...
	inline bool AllowIndirectLightingCache() const
	{
		return true;
	}

	inline bool AllowIndirectLightingCacheVolumeTexture() const
	{
		return CLUSTERED_SUPPORT_VOLUME_INDIRECT;
	}
...
bool FClusteredForwardShadingSceneRenderer::RenderForwardShadingBasePassView(FRHICommandListImmediate& RHICmdList, const FViewInfo& View, ESceneDepthPriorityGroup DepthPriorityGroup)
{
	...
	auto DrawStaticPrimitives = [this, &RHICmdList, DepthPriorityGroup, SortMode](const FViewInfo& View, EBasePassDrawListType DrawType) -> bool
	{
		bool bDirty = false;
	...
	if (EarlyZPassMode != DDM_None)
	{
...

		{
			...
			bDirty |= DrawStaticPrimitives(View, EBasePass_Masked);
		}
		{
			SCOPED_DRAW_EVENT(RHICmdList, Static);
			bDirty |= DrawStaticPrimitives(View, EBasePass_Default);
		}
	}
	else
	{
...
#define POLICY_TYPE(TYPE) \
	...
	POLICY_TYPE( LMP_MDL_CSM_SHINDVL )
	POLICY_TYPE( LMP_MDL_SHINDPT )
	POLICY_TYPE( LMP_MDL_SHINDVL )
	POLICY_TYPE( LMP_SHINDPT )
	POLICY_TYPE( LMP_SHINDVL )
