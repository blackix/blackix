...
// If spot lights are enabled or not (will trim some branching/regs from the shaders)
#define CLUSTERED_SUPPORT_SPOT_LIGHTS 0

// If we support the volume indirect lighting cache.
#define CLUSTERED_SUPPORT_VOLUME_INDIRECT (true && !CLUSTERED_FAST_ITERATION)


/**
 * Shared #defines for all clustered shaders
 */
struct ClusteredShadingShaderCommon
{
	static void ModifyCompilationEnvironment(EShaderPlatform Platform, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		OutEnvironment.SetDefine(TEXT("CLUSTERED_SUPPORT_SPOT_LIGHTS"), uint32(CLUSTERED_SUPPORT_SPOT_LIGHTS));
	}
};
...
public:
	...
	FDrawingPolicyMatchResult Matches(const TBasePassForClusteredShadingDrawingPolicy& Other) const
	{
		DRAWING_POLICY_MATCH_BEGIN
			...
			DRAWING_POLICY_MATCH(BlendMode == Other.BlendMode) &&
			DRAWING_POLICY_MATCH(bEnableSkyLight == Other.bEnableSkyLight) &&
			DRAWING_POLICY_MATCH(bEnableReflectionProbe == Other.bEnableReflectionProbe) &&
			DRAWING_POLICY_MATCH(bEnableAlphaToCoverage == Other.bEnableAlphaToCoverage);
	...
	)
{
	...
	if (bIsLitMaterial)
	{
			...
			&& Parameters.Mesh.LCI->GetLightMapInteraction(Parameters.FeatureLevel).GetType() == LMIT_Texture)
		{
			...
			&& Parameters.PrimitiveSceneProxy->GetIndirectLightingCacheQuality() != ILCQ_Off)
		{
			#if CLUSTERED_SUPPORT_VOLUME_INDIRECT
			...
			if (CanIndirectLightingCacheUseVolumeTexture(Parameters.FeatureLevel)
				...
				&& ((IndirectLightingCacheAllocation && !IndirectLightingCacheAllocation->bPointSample)
					|| (Parameters.PrimitiveSceneProxy->GetIndirectLightingCacheQuality() == ILCQ_Volume)))
			{
				if (bHasCSM)
				{
					// Light+CSM+SHIndirectVolume
					Action.template Process<FUniformLightMapPolicy>(RHICmdList, Parameters, FUniformLightMapPolicy(LMP_MDL_CSM_SHINDVL), Parameters.Mesh.LCI);
				}
				else if (bHasLight)
				{
					// Light+SHIndirectVolume
					Action.template Process<FUniformLightMapPolicy>(RHICmdList, Parameters, FUniformLightMapPolicy(LMP_MDL_SHINDVL), Parameters.Mesh.LCI);
				}
				else
				{
					// SHIndirectVolume
					Action.template Process<FUniformLightMapPolicy>(RHICmdList, Parameters, FUniformLightMapPolicy(LMP_SHINDVL), Parameters.Mesh.LCI);
				}
			}
			else
			#endif /* CLUSTERED_SUPPORTS_VOLUME_INDIRECT */
			{
				if (bHasCSM)
				{
