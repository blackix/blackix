...
/// Gets the current initialization state of the Oculus runtime, VR tracking, and graphics resources.
OVRP_EXPORT ovrpBool ovrp_GetInitialized();

/// Sets up the Oculus runtime, VR tracking, and graphics resources.
/// You must call this before any other function except ovrp_PreInitialize() or ovrp_GetInitialized().
OVRP_EXPORT ovrpResult ovrp_Initialize3(ovrpRenderAPIType apiType, ovrpLogCallback logCallback, void* activity, int initializeFlags, const ovrpVersion& version = { OVRP_VERSION });
...
/// @note ovrp_PreInitialize must be called and return a successful result before calling this function.
...
OVRP_EXPORT ovrpResult ovrp_SetupLayer(void* device, const ovrpLayerDesc& desc, int* layerId);
...
OVRP_EXPORT ovrpResult ovrp_GetLayerOcclusionMesh(int layerId, ovrpEye eyeId, ovrpVector2f const** vertices, int* vertexCount, int const** indices, int* indexCount);

/// Destroys a layer
OVRP_EXPORT ovrpResult ovrp_DestroyLayer(int layerId);

/// Calculates layer description
OVRP_EXPORT ovrpResult ovrp_CalculateLayerDesc(ovrpShape shape, ovrpLayout layout, const ovrpSizei& textureSize, int mipLevels, int sampleCount, ovrpTextureFormat format, int layerFlags, ovrpLayerDescUnion* layerDesc);

/// Calculates eye layer description
OVRP_EXPORT ovrpResult ovrp_CalculateEyeLayerDesc(ovrpLayout layout, float textureScale, int mipLevels, int sampleCount, ovrpTextureFormat format, int layerFlags, ovrpLayerDesc_EyeFov* layerDesc);

/// Calculates the recommended viewport rect for the specified eye
OVRP_EXPORT ovrpResult ovrp_CalculateEyeViewportRect(const ovrpLayerDesc_EyeFov& layerDesc, ovrpEye eyeId, float viewportScale, ovrpRecti* viewportRect);

/// Calculates the area of the viewport unobstructed by the occlusion mesh
OVRP_EXPORT ovrpResult ovrp_CalculateEyePreviewRect(const ovrpLayerDesc_EyeFov& layerDesc, ovrpEye eyeId, const ovrpRecti& viewportRect, ovrpRecti* previewRect);
...
/// a texture ID allocated by glGenTextures in the device argument and the texture will be associated
/// with that ID.
OVRP_EXPORT ovrpResult ovrp_SetupMirrorTexture2(void* device, int height, int width, ovrpTextureFormat format, ovrpTextureHandle* textureHandle);
...
/// Call with ovrpStep_Physics from any thread, using predictionSeconds specify offset from start of frame.
...
/// Marks the end of a frame and performs TimeWarp. Call this before Present or SwapBuffers to update the VR window.
OVRP_EXPORT ovrpResult ovrp_EndFrame2(int frameIndex, ovrpLayerSubmit const* const* layerSubmitPtrs, int layerSubmitCount);
...
/// Oculus Internal. Sets the system-wide calibrated origin for the currently active tracking origin type.
...
/// Force a node position to be tracked or not. Return false if the node's position tracking cannot be changed.
...
OVRP_EXPORT ovrpResult ovrp_GetControllerHapticsDesc2(ovrpController controllerMask, ovrpHapticsDesc* controllerHapticsDesc);

/// Gets the current haptics state for the given controllers.
OVRP_EXPORT ovrpResult ovrp_GetControllerHapticsState2(ovrpController controllerMask, ovrpHapticsState* controllerHapticsState);
...
/// Gets the physical distance from the base of the neck to the center of the player's eyes in meters.
OVRP_EXPORT ovrpResult ovrp_GetUserNeckEyeDistance2(ovrpVector2f* userEyeNeckDistance);

/// Sets the physical distance from the base of the neck to the center of the player's eyes in meters.
...
OVRP_EXPORT ovrpResult ovrp_TestBoundaryNode2(ovrpNode node, ovrpBoundaryType boundaryType, ovrpBoundaryTestResult* boundaryTestResult);

/// Performs a boundary test between the specified point and boundary types.
OVRP_EXPORT ovrpResult ovrp_TestBoundaryPoint2(ovrpVector3f point, ovrpBoundaryType boundaryType, ovrpBoundaryTestResult* boundaryTestResult);

/// Configures the boundary system's look and feel with the provided configuration data. May be overridden by user settings.
...
/// Gets the dimensions for the specified boundary type. Returned x,y,z values correspond to width, height, depth.
...
/// Requests that the boundary system visibility be set to the specified value. Can be overridden by the boundary system or the user.
...
/// Resets internal performance counters to clear previous data from impacting the current reported state. 
