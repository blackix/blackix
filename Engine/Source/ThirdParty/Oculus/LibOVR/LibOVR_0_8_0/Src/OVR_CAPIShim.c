...
//
    ...
    #define OVR_ENABLE_DEVELOPER_SEARCH 1
...
OVR_PUBLIC_FUNCTION(ovrResult) ovr_Initialize(const ovrInitParams* inputParams)
{
    ...
    params.Flags |= ovrInit_SingleProcess;
#endif // OVR_SINGLE_PROCESS
#endif // OVR_INTERNAL_CODE

#ifdef OVR_INTERNAL_CODE
    // Set the internal code flag to let the runtime DLL know what feature level CAPI header is being used by the client.
    params.Flags |= ovrInit_InternalCode;
#endif // OVR_INTERNAL_CODE
#ifdef OVR_PARTNER_CODE
    // Set the partner code flag to let the runtime DLL know what feature level CAPI header is being used by the client.
    params.Flags |= ovrInit_PartnerCode;
