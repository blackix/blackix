...
OVR_USES_REDACTION
    ...
    /* Instructions                                                             */
    /*                                                                          */
    /* Error codes identify OVR-level errors returned by public API functions.  */
    /* All LibOVR errors are prefixed by ovrError_ and the rest of the name is  */
    /* camel-cased.                                                             */
    /*                                                                          */
    /* Errors codes are in functional groupings. If you need to add a new       */
    /* functional group then you should start one with a new 1000-based value.  */
    /* Once an error is added, it cannot be removed or have its value           */
    /* renamed or repurposed.                                                   */
    /*                                                                          */
    /* These errors are OVR-level errors are not low-level platform-specific    */
    /* error such as errno or Windows HRESULT errors. It may well be that a     */
    /* lower level platform error was the original cause of an OVR-level error. */
    /* Such lower level platform errors are captured by the internal OVRError   */
    /* class and are logged automatically if you call OVR_MAKE_SYS_ERROR.       */
    /*                                                                          */
    /* Errors are declared in an enum, but all errors don't need to be in the   */
    /* same enum, but can be in separate per-subsystem enums. However, any      */
    /* errors that will be returned by public APIs must be publicly visible.    */
    /*                                                                          */
    /* All error declarations currently should be matched by an entry in        */
    /* errorDescriptionArray from OVR_Error.cpp so that they can be logged by   */
    /* name instead of just by number.                                          */
    /*                                                                          */
    /* Some ovrResult return values may be informative as opposed to being      */
    /* formal errors. There are sometimes cases where the return value of a     */
    /* function may or may not be an error depending on what the user needs.    */
    /*                                                                          */
    /* An error code of the type "unknown" (e.g. ovrError_Unknown) is not       */
    /* allowed. All errors must be known, else we have lost track of what the   */
    /* error is.                                                                */
