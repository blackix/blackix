...
typedef enum ovrHmdCaps_
{
    ...
    ovrHmdCap_DynamicPrediction       = 0x0200,   ///< \internal <B>(read/write)</B> Adjusts prediction dynamically based on internally measured latency.
    ovrHmdCap_DirectPentile           = 0x0400,   ///< \internal <B>(read/write)</B> Writes directly in PenTile color mapping format

    // 0x0800 free for use (was ovrHmdCap_PanelUniformMuraEnable)
    // 0x1000 free for use (was ovrHmdCap_PanelUniformEnable)
    ovrHmdCap_ComputeShaderEnable     = 0x2000,   ///< \internal <B>(read/write)</B> Enable compute shader distortion (defaults off).
    ...
    ovrHmdCap_Writable_Mask       = ovrHmdCap_LowPersistence         |
                                    ...
                                    ovrHmdCap_DirectPentile          |
                                    ovrHmdCap_ComputeShaderEnable,
...
{    
    ...
    ovrButton_VolUp     = 0x00400000,  // only in SID, so hidden
    ovrButton_VolDown   = 0x00800000,  // only in SID, so hidden
...
typedef enum ovrInitFlags_
{
    ...
    /// This flag is deprecated. Please don't write any new code to use it and instead
    /// call ovr_Initialize periodically or poll ovr_Detect if you want to identify when 
    /// OVRService is available.
    ...
    /// In single-process mode, the OVR lib also hosts the tracking server components.
    /// This is incompatible with the released Runtime, but is useful for debugging tracking.
    ovrInit_SingleProcess = 0x00001000,
#endif // OVR_PARTNER_CODE
#ifdef OVR_INTERNAL_CODE
    /// Usually a headset requires both the USB sensor device and the display panel connected
    /// over HDMI. However, for internal testing, you might want to run with
    /// only the sensor board attached. Currently, this works only in single-process mode.
    ovrInit_HDMIOptional   = 0x00010000,

    /// Usually, a headset and camera must be calibrated before being used with the
    /// SDK. However, for internal tools, we want the flexibility to work with sensors and cameras
    /// that are not yet calibrated. Currently, this only works in single-process mode.
    ovrInit_NoCalibration  = 0x00020000,
#endif // OVR_INTERNAL_CODE
#ifdef OVR_PARTNER_CODE
    // This flag indicates that the game is using the Partner version of this header file.
    ovrInit_PartnerCode    = 0x01000000,
#endif // OVR_PARTNER_CODE
#ifdef OVR_INTERNAL_CODE
    // This flag indicates that the game is using the Internal version of this header file.
    ovrInit_InternalCode   = 0x02000000,
#endif // OVR_INTERNAL_CODE
...
#ifdef OVR_INTERNAL_CODE
    ovrPerfHud_PredictionFidelity = 5,  ///< Shows tracking prediction fidelity info
#endif // OVR_INTERNAL_CODE
...
#ifdef OVR_INTERNAL_CODE
OVR_STATIC_ASSERT(ovrDebugHudStereo_Off == 0, "ovrDebugHudStereo_Off must be 0.");
#endif // OVR_INTERNAL_CODE
