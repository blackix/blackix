...
{
	my ($relative_url) = @_;
	my $server_url = $ENV{'COMMANDER_SERVER'} || 'ec-01.epicgames.net';
	"https://$server_url$relative_url";
}
...
sub ec_parse_job
{
	...
	$job->{'started_by'} = $job_xpath->findvalue("./launchedByUser", $job_node)->string_value();
...
sub ec_parse_jobstep
{
	...
	$jobstep->{'jobstep_log'} = $log_file_name;
...
# allocates an execution slot
sub allocate_slot
{
	my ($root_dir, $num_slots) = @_;
	
	for(my $attempt = 1; $attempt < 100; $attempt++)
	{
		for(my $idx = 1; $idx <= $num_slots; $idx++)
		{
			my $slot_file = sprintf("$root_dir/slot-%02d", $idx);

			open(SLOT_FILE, ">$slot_file") or next; 
			if(!flock(SLOT_FILE, LOCK_EX | LOCK_NB))
			{
				close(SLOT_FILE);
				next;
			}

			print "Using slot $idx\n";
			return $idx;
		}

		print "Unable to find unused slot; pausing 1s and retrying.\n";
		sleep 1;
	}

	die "Unable to allocate slot";
}
...
sub mkdir_recursive
{
	...
	if(!-d $dir)
	{
		...
		if(!-d $dir)
		{
			...
			{
				fail("Couldn't create $dir ($result): $!");
			}
		}
	}
}
...
sub safe_delete_directory
{
	...
	if(-d $directory_name)
	{
		# use a special program that can handle deleting paths > MAX_PATH characters long on Windows. Source for this program is in Engine/Extras/UnsupportedTools.
		my $safe_delete_directory = 'SafeDeleteDirectory.exe';
		if(is_windows() && -f $safe_delete_directory)
		{
			system("$safe_delete_directory \"$directory_name\"");
			return if !-d $directory_name;
		}
...
# delete the contents of a directory, including all read-only files
sub safe_delete_directory_contents
{
	my $directory_name = shift;
	if(-d $directory_name)
	{
		find({ wanted => \&safe_delete_file_or_directory, bydepth => 1 }, $directory_name);
	}
}

# delete a directory, including all read-only files
sub safe_delete_file_or_directory
{
	if($_ ne '.' && $_ ne '..')
	{
		my $file_or_directory_name = $_;
		if(-d $file_or_directory_name)
		{
			safe_delete_directory($file_or_directory_name);
		}
		else
		{
			safe_delete_file($file_or_directory_name);
		}
	}
}
...
# delete a directory, including all read-only files
sub try_delete_directory_contents
{
	my $directory_name = shift;
	if(-d $directory_name)
	{
		find({ wanted => \&try_delete_file_or_directory, bydepth => 1 }, $directory_name);
	}
}

# delete a directory, including all read-only files
sub try_delete_file_or_directory
{
	if($_ ne '.' && $_ ne '..')
	{
		my $file_or_directory_name = $_;#shift;
		if(-d $file_or_directory_name)
		{
			rmtree($file_or_directory_name);
			print "Unable to delete $file_or_directory_name\n" if -d $file_or_directory_name;
		}
		else
		{
			chmod 0777, $file_or_directory_name;
			unlink($file_or_directory_name);
			print "Unable to delete $file_or_directory_name\n" if -f $file_or_directory_name;
		}
	}
}
...
# gets a time value representing midnight on sunday of the provided date/time. forces time zone to EST.
sub get_day_of_week
{
	my ($time) = @_;
...
sub format_recent_time
{
	...
	my $midnight_tomorrow = $midnight_today + (24 * 60 * 60);
	my $midnight_day_after_tomorrow = $midnight_tomorrow + (24 * 60 * 60);
	my $midnight_yesterday = $midnight_today - (24 * 60 * 60);
	my $this_week = $midnight_today - (6 * 24 * 60 * 60);
	my $next_week = $midnight_today + (7 * 24 * 60 * 60);

	my ($time_seconds, $time_minutes, $time_hours, $date_day, $date_month, $date_year, $date_dow) = localtime($datetime);
	my $time_string = sprintf "%d:%02d %s", (($time_hours + 11) % 12) + 1, $time_minutes, ($time_hours < 12)? "am" : "pm";
	
	if($datetime > $next_week)
	{
		($date_month + 1)."/$date_day/".($date_year + 1900)." at $time_string";
	}
	elsif($datetime > $midnight_day_after_tomorrow)
	{
		('Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday')[$date_dow].' '.($date_month + 1)."/$date_day at $time_string";
	}
	elsif($datetime > $midnight_tomorrow)
	{
		"Tomorrow at $time_string";
	}
	elsif($datetime > $midnight_today)
	{
		$time_string;
	}
