...
sub update_dashboard
{
	...
	push(@{$filter}, { propertyName => 'HideFromDashboard', operator => 'notEqual', operand1 => '1' });
...
sub update_settings
{
	...
	# store the agent type -> resource pool mapping for easy access on new jobs
	foreach my $agent_type(keys %{$settings->{'Agent Types'}})
	{
		my $resource_pool = $settings->{'Agent Types'}->{$agent_type}->{'Resource Pool'};
		if($resource_pool)
		{
			ec_set_property($ec, "/projects[$ec_project]/Generated/".escape_stream_name($stream)."/Agent Types/$agent_type", $resource_pool, $ec_update);
		}
	}
}
	...
	if($build_types)
	{
		foreach my $build_type(@{$build_types})
		{
			if($build_type->{'ShowLink'})
			{
				...
				$link .= "&parameters4_name=".encode_form_parameter("Initial Agent Type");
				$link .= "&parameters4_value=".encode_form_parameter($build_type->{'Initial Agent Type'} || "");
				$link .= "&parameters5_name=".encode_form_parameter("Semaphores");
				$link .= "&parameters5_value=".encode_form_parameter($build_type->{'Semaphores'} || "");
...
sub update_preflight_links
{
	...
	if($build_types)
	{
		foreach my $build_type(@{$build_types})
		{
			...
			{
				my $name = $build_type->{'Name'};
				my $link = get_preflight_url($ec_project, $stream, encode_form_parameter($name), encode_form_parameter($build_type->{'Arguments'}), encode_form_parameter($build_type->{'Initial Agent Type'} || ""), encode_form_parameter($build_type->{'Semaphores'} || ""));
...
{
	my ($ec_project, $stream, $name, $arguments, $initial_agent_type, $semaphores) = @_;
	...
	$url .= "&parameters6_name=".encode_form_parameter("Initial Agent Type");
	$url .= "&parameters6_value=".encode_form_parameter($initial_agent_type);
	$url .= "&parameters7_name=".encode_form_parameter("Semaphores");
	$url .= "&parameters7_value=".encode_form_parameter($semaphores);
...
sub update_telemetry
{
	...
	foreach my $job_id (@{$job_ids})
	{
		...
		if($job_properties->{'build_info'})
		{
			...
			# flag for whether the job can run early
			my $node_name_to_run_early = {};
			my $run_early_nodes = $job_properties->{'build_info'}->{'RunEarly'};
			if($run_early_nodes)
			{
				foreach my $node_name (@{$run_early_nodes})
				{
					$node_name_to_run_early->{$node_name} = 1;
				}
			}
			...
			if($node_name_to_dependencies)
			{
				...
				foreach my $jobstep (@{$job_properties->{'job_steps'}})
				{
					...
					if($result =~ /^completed:(success|warning)/ && $jobstep->{'run_time'})
					{
						...
						my $run_early = $node_name_to_run_early->{$name} || 0;
						my $user_data = $jobstep->{'telemetry'};
						
						my $jobstep_telemetry = { id => $jobstep_id, name => $name, group => $group, start_time => $start_time, finish_time => $finish_time, depends_on => $depends_on, run_early => $run_early };
...
sub update_dashboard_grid_view
{
	...
	for my $job_id (@{$job_ids})
	{
		...
		{
			$image = "<img src=\"/commander/lib/images/icn16px_$job_properties->{'outcome'}.gif\" width=\"16px\" height=\"16px\" />";
		}
		elsif($job_properties->{'status'} eq 'running')
		{
			$image = "<img src=\"/commander/lib/images/icn16px_running_$job_properties->{'outcome'}.gif\" width=\"16px\" height=\"16px\" />";
		}
		elsif($job_properties->{'status'} eq 'pending' || $job_properties->{'status'} eq 'runnable')
		{
			$image = "<img src=\"/commander/lib/images/icn16px_runnable.gif\" width=\"16px\" height=\"16px\" />";
		}

		$XHTML .= "<td bgcolor=#C8C8CE width=\"65px\" align=\"center\" style=\"padding-top:5px;padding-bottom:2px;padding-left:0.75em;padding-right:0.75em;line-height:150%;\" title=\"$started_time\">$image<br><a href=/commander/link/jobDetails/jobs/$job_id\><strong>$heading</strong></a></td>";
	}
	$XHTML .= "  </tr>\n";
...
sub update_dashboard_list_view
{
	...
	# remove all the blacklisted job names as specified by the settings file
	my $list_settings = $settings && $settings->{'Dashboard'} && $settings->{'Dashboard'}->{'List'};
	if($list_settings)
	{
		my $exclude_jobs = $list_settings->{'Exclude Jobs'};
		if($exclude_jobs)
		{
			# create a new array of job ids that don't match any exclude patterns
			my $new_job_ids = [];
	...
	foreach my $job_id (@sorted_job_ids)
	{
		...
		my $p4swarm_url = "http://p4-swarm.epicgames.net/files/".substr($stream, 1)."?range=\@$change_number#commits";
