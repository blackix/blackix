...
sub execute_command
{
	...
	*STDERR = *STDOUT;
	...
	# remove any undefined or empty entries from the argument list. calling quotewords on string that ends in empty entries can end up with these.
	$raw_arguments = [grep { $_ } @{$raw_arguments}];
	...
	if(is_windows())
	{
		my $temp_dir = join_paths($root_dir, "Temp");
		print "Clearing temp directory ($temp_dir)\n";
		mkdir_recursive($temp_dir);
		try_delete_directory_contents($temp_dir);
	...
	elsif($command eq 'buildsetup')
	{
		...
		ec_set_property($ec, "/myJobStep/CL", "$change", $ec_update);

		if($arguments->{'hide-from-dashboard'})
		{
			ec_set_property($ec, "/jobs[$ec_job]/HideFromDashboard", 1, $ec_update);
		}
		...
		{
			$pass_through_arguments .= " -reportName=\"Build Summary\"" if ec_get_property($ec, "/jobs[$ec_job]/launchedByUser") =~ /^[^ ]+\@[^ ]+$/;
		}
		...
		my $agent_type = $arguments->{'agent-type'} || $settings->{'Initial Agent Type'};
		my $workspace = setup_workspace($root_dir, $stream, { ec => $ec, ec_project => $ec_project, agent_type => $agent_type });
		terminate_processes($workspace, $root_dir);
		...
		build_setup($ec, $ec_project, $ec_jobstep, $workspace, $change, $code_change, $build_script, $target, $trigger, $token_signature, $shared_storage_dir, $pass_through_arguments, { email_only => $arguments->{'email-only'} }, $ec_update);
		my $uat_time = time;
		build_job_setup($ec, $ec_project, $ec_job, $ec_update, $settings, $workspace, $agent_type, $change, $build_script, $target, $token_signature, $shared_storage_block, $arguments, $pass_through_arguments, { email_only => $arguments->{'email-only'} });
	...
	elsif($command eq 'buildagentsetup')
	{
		...
		safe_delete_directory(join_paths($workspace->{'dir'}, 'Engine', 'Saved', 'BuildGraph'));
		my $sync_time = time;
	...
	elsif($command eq 'buildparallelsetup')
	{
		my $stream = get_required_parameter($arguments, 'stream');
		my $agent_type = get_required_parameter($arguments, 'agent-type');
		my $change = get_required_parameter($arguments, 'change');
		...
		my $workspace = undef;
		build_agent_setup($ec, $ec_job, $workspace, $change, $code_change, $build_script, $target, $group, $token_signature, $resource_name, $shared_storage_dir, $ec_update, { fake_build => $arguments->{'fake-build'}, fake_fail => $arguments->{'fake-fail'}, pass_through_arguments => $pass_through_arguments, temp_storage_dir => $arguments->{'temp-storage-dir'}, email_only => $arguments->{'email-only'}, autosdk => $arguments->{'autosdk'}, parallel => 1, stream => $stream, agent_type => $agent_type, preflight_change => $arguments->{'preflight-change'} });
	}
	elsif($command eq 'buildparallelnode')
	{
		my $stream = get_required_parameter($arguments, 'stream');
		my $agent_type = get_required_parameter($arguments, 'agent-type');
		my $change = get_required_parameter($arguments, 'change');
		...
		my $build_script = get_required_parameter($arguments, 'build-script');
		my $token_signature = get_required_parameter($arguments, 'token-signature');
		my $num_slots = get_required_parameter($arguments, 'num-slots');
		my $node = get_required_parameter($arguments, 'node');

		my $slot_idx = allocate_slot($root_dir, $num_slots);
		ec_set_property($ec, "/myJobStep/postSummary", "Hosted by $resource_name (slot $slot_idx)", $ec_update);
		...
		my $workspace = setup_workspace($root_dir, $stream, { ec => $ec, ec_project => $ec_project, agent_type => $agent_type, slot_idx => $slot_idx });
		terminate_processes($workspace, $workspace->{'dir'});
		sync_workspace($workspace, { change => $change, unshelve_change => $arguments->{'preflight-change'}, show_traffic => $arguments->{'show-traffic'} }) unless $arguments->{'skip-sync'};
		print "Copying UAT binaries...\n";
		copy_recursive(join_paths(getcwd(), "UAT"), $workspace->{'dir'});
		safe_delete_directory(join_paths($workspace->{'dir'}, 'Engine', 'Saved', 'BuildGraph'));
		...
		build_single_node($ec, $ec_project, $ec_job, $ec_jobstep, $stream, $change, $workspace->{'name'}, $workspace->{'dir'}, $build_script, $node, $token_signature, $ec_update, { fake_build => $arguments->{'fake-build'}, fake_fail => $arguments->{'fake-fail'}, email_only => $arguments->{'email-only'}, pass_through_arguments => $pass_through_arguments });
	...
	elsif($command eq 'conformresources')
	{
		...
		my $delete_folders = $arguments->{'delete-folders'};
		my $p4_clean = $arguments->{'p4-clean'};
		my $max_parallel = $arguments->{'max-parallel'};
		conform_resources($ec, $ec_project, $ec_job, $ec_jobstep, $names, $set_pools, $delete_folders, $p4_clean, $max_parallel, $ec_update);
	}
	elsif($command eq 'conformresource')
	{
		...
		my $p4_clean = $arguments->{'p4-clean'};
		conform_resource($ec, $ec_project, $name, $root_dir, $delete_folders, $p4_clean, $num_slots);
	}
	...
	elsif($command eq 'copylog')
	{
		my $ec_jobstep = get_required_parameter($arguments, 'ec-jobstep');
		my $to_file = $arguments->{'to-file'};

		my $jobstep = ec_get_jobstep($ec, $ec_jobstep);

		my $from_file = join_paths($jobstep->{'workspace_dir'}, $jobstep->{'jobstep_log'});
		$to_file = $to_file || $jobstep->{'jobstep_log'};

		fail("Unable to find $from_file") if !-f $from_file;
...
sub get_latest_code_change
{
	...
	# find the last change submitted by a user
	my @change_lines = p4_command("-c $workspace_name changes -m 1 //$workspace_name/....cpp\@$change //$workspace_name/....h\@$change //$workspace_name/....inl\@$change //$workspace_name/....cs\@$change //$workspace_name/....usf\@$change //$workspace_name/....ush\@$change //$workspace_name/....uproject\@$change //$workspace_name/....uplugin\@$change");
