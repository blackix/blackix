...
void AOculusMR_CastingCameraActor::Tick(float DeltaTime)
{
	...
	if (HasCastingWindowOpened())
	{
		...
		if (ChromaKeyMaterialInstance)
		{
			ChromaKeyMaterialInstance->SetVectorParameterValue(FName(TEXT("ChromaKeyColor")), FLinearColor(ChromaKeyColor));
			ChromaKeyMaterialInstance->SetScalarParameterValue(FName(TEXT("ChromaKeySimilarity")), ChromaKeySimilarity);
			ChromaKeyMaterialInstance->SetScalarParameterValue(FName(TEXT("ChromaKeySmoothRange")), ChromaKeySmoothRange);
			ChromaKeyMaterialInstance->SetScalarParameterValue(FName(TEXT("ChromaKeySpillRange")), ChromaKeySpillRange);
		}
...
void AOculusMR_CastingCameraActor::UpdateCameraColorTexture(const ovrpSizei &frameSize, const ovrpByte* frameData, int rowPitch)
{
	if (CameraColorTexture->GetSizeX() != frameSize.w || CameraColorTexture->GetSizeY() != frameSize.h)
	{
		...
		if (ChromaKeyMaterialInstance)
		{
			ChromaKeyMaterialInstance->SetTextureParameterValue(FName(TEXT("CameraCaptureTexture")), CameraColorTexture);
			ChromaKeyMaterialInstance->SetVectorParameterValue(FName(TEXT("CameraCaptureTextureSize")), 
...
void AOculusMR_CastingCameraActor::OpenCastingWindow()
{
	...
	if (CompositionMethod == ECastingViewportCompositionMethod::DirectComposition)
	{
		...
		PlaneMeshComponent->SetMaterial(0, ChromaKeyMaterialInstance);
