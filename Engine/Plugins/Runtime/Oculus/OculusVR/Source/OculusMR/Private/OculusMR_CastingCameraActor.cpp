...
void AOculusMR_CastingCameraActor::SetTrackedCameraInitialPoseWithPlayerTransform()
{
	...
	CameraComponent->FieldOfView = TrackedCamera.FieldOfView;
}
...
void AOculusMR_CastingCameraActor::SetTrackedCameraUserPoseWithCameraTransform()
{
	...
	FPose ExpectedCameraPose(CameraComponent->GetComponentRotation().Quaternion(), CameraComponent->GetComponentLocation());
...
void AOculusMR_CastingCameraActor::RepositionPlaneMesh()
{
	if (CameraColorTexture)
	{
		...
		float Width = ClippingPlaneDistance * FMath::Tan(FMath::DegreesToRadians(CameraComponent->FieldOfView) * 0.5f) * 2.0f;
