...
//			UE_LOG(LogHMD, Log, TEXT("Splash ovrp_BeginFrame4 %u"), XFrame->FrameNumber);
			if (OVRP_FAILURE(result = ovrp_BeginFrame4(XFrame->FrameNumber, CustomPresent->GetOvrpCommandQueue())))
			{
				UE_LOG(LogHMD, Error, TEXT("Splash ovrp_BeginFrame4 %u failed (%d)"), XFrame->FrameNumber, result);
				XFrame->ShowFlags.Rendering = false;
			}
		}
...
//			UE_LOG(LogHMD, Log, TEXT("Splash ovrp_EndFrame4 %u"), XFrame->FrameNumber);
			if (OVRP_FAILURE(result = ovrp_EndFrame4(XFrame->FrameNumber, LayerSubmitPtr.GetData(), LayerSubmitPtr.Num(), CustomPresent->GetOvrpCommandQueue())))
			{
				UE_LOG(LogHMD, Error, TEXT("Splash ovrp_EndFrame4 %u failed (%d)"), XFrame->FrameNumber, result);
			}
			else
			{
				for (int32 LayerIndex = 0; LayerIndex < Layers_RHIThread.Num(); LayerIndex++)
