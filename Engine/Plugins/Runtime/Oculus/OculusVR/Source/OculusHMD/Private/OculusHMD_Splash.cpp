...
void FSplash::Tick_RenderThread(float DeltaTime)
{
	...
	if(DeltaTimeInSeconds > 2.f * SystemDisplayFrequency)
	{
		FScopeLock ScopeLock(&RenderThreadLock);
...
void FSplash::Startup()
{
	...
	if (!bInitialized)
	{
		...
		FCoreUObjectDelegates::PostLoadMap.AddSP(this, &FSplash::OnPostLoadMap);
...
bool FSplash::PushFrame()
{
...

	{
		...
		ovrp_GetSystemDisplayFrequency2(&SystemDisplayFrequency);
	}
