...
void FOculusHMD::CalculateStereoViewOffset(const EStereoscopicPass StereoPassType, const FRotator& ViewRotation, const float WorldToMeters, FVector& ViewLocation)
{
	...
	if (CurrentFrame && (StereoPassType != eSSP_FULL || CurrentSettings->Flags.bHeadTrackingEnforced))
	{
		...
		if (bIsInGameThread && StereoPassType == eSSP_LEFT_EYE) // this is per-frame, shouldn't run per-view and definitively not per-thread
		{
			CurrentFrame->PlayerLocation = LastPlayerLocation = ViewLocation;
		}

		const int ViewIndex = ViewIndexFromStereoPass(StereoPassType);

		if (CurrentFrame->Flags.bPlayerControllerFollowsHmd)
		{
			// Subtract out GameHeadPose.Position PlayerLocation already contains head position
//			PlayerLocation -= PlayerOrientation.RotateVector(CurrentFrame->GameHeadPose.Position);
			ViewLocation = CurrentFrame->PlayerLocation + CurrentFrame->PlayerOrientation.RotateVector(CurrentFrame->EyePose[ViewIndex].Position - CurrentFrame->HeadPose.Position) + CurrentSettings->PositionOffset;
		}
		else
		{
			ViewLocation = CurrentFrame->PlayerLocation + CurrentFrame->PlayerOrientation.RotateVector(CurrentFrame->EyePose[ViewIndex].Position) + CurrentSettings->PositionOffset;
		}
...
void FOculusHMD::RenderTexture_RenderThread(class FRHICommandListImmediate& RHICmdList, class FRHITexture2D* BackBuffer, class FRHITexture2D* SrcTexture) const
{
	...
	if (MirrorWindowMode != EMirrorWindowMode::Disabled)
	{
		if (MirrorWindowMode == EMirrorWindowMode::Distorted)
		{
			...
			if (MirrorTexture)
			{
				CustomPresent->CopyTexture_RenderThread(RHICmdList, BackBuffer, MirrorTexture, MirrorTexture->GetTexture2D()->GetSizeX(), MirrorTexture->GetTexture2D()->GetSizeY());
			}
		}
		else if (MirrorWindowMode == EMirrorWindowMode::Undistorted)
		{
			...
			for (int i = 0; i < 2; ++i)
			{
				CustomPresent->CopyTexture_RenderThread(RHICmdList, BackBuffer, SrcTexture, SrcTexture->GetTexture2D()->GetSizeX(), SrcTexture->GetTexture2D()->GetSizeY(), destRect, Settings_RenderThread->EyeRenderViewport[i]);
			...
			MirrorWindowMode == EMirrorWindowMode::SingleEyeCroppedToFill)
		{
			...
			FIntRect SrcViewRect(Settings_RenderThread->EyeRenderViewport[0]);
			...
			CustomPresent->CopyTexture_RenderThread(RHICmdList, BackBuffer, SrcTexture, SrcTexture->GetTexture2D()->GetSizeX(), SrcTexture->GetTexture2D()->GetSizeY(), DstViewRect, SrcViewRect, false, false);
		}
	}
...
void FOculusHMD::UseImplicitHmdPosition(bool bInImplicitHmdPosition)
{
	...
	if (NextFrameToRender.IsValid())
	{
		...
		this->bImplicitHmdPosition = bInImplicitHmdPosition;
	}
}
...
void FOculusHMD::ApplyHmdRotation(APlayerController* PC, FRotator& ViewRotation)
{
	...
	Frame->PlayerOrientation = DeltaControlOrientation;
...
bool FOculusHMD::UpdatePlayerCamera(FQuat& HeadOrientation, FVector& HeadPosition)
{
	...
	if (Settings->Flags.bDoNotUpdateOnGT)
	{
		Frame->Flags.bOrientationChanged = Settings->Flags.bPlayerCameraManagerFollowsHmdOrientation;// POV.bFollowHmdOrientation;
		Frame->Flags.bPositionChanged = Settings->Flags.bPlayerCameraManagerFollowsHmdPosition;//POV.bFollowHmdPosition;
		return false;
	}
#endif
...
bool FOculusHMD::EnablePositionalTracking(bool enable)
{
	return true;
}

bool FOculusHMD::IsInLowPersistenceMode() const
{
	return true;
}

void FOculusHMD::EnableLowPersistenceMode(bool Enable)
{
	return;
}
...
bool FOculusHMD::Exec(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	return false;
}
...
void FOculusHMD::FinishRHIFrame_RHIThread()
{
	...
	if (Frame_RHIThread.IsValid() && Frame_RHIThread->ShowFlags.Rendering && !Frame_RHIThread->Flags.bSplashIsShown)
	{
		...
		Layers.Sort(FLayerPtr_ComparePriority());
