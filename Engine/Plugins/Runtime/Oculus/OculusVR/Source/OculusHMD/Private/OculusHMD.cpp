...
void FOculusHMD::RenderTexture_RenderThread(class FRHICommandListImmediate& RHICmdList, class FRHITexture2D* BackBuffer, class FRHITexture2D* SrcTexture) const
{
	...
	if (MirrorWindowMode != EMirrorWindowMode::Disabled)
	{
        ...
        {
            CustomPresent->CopyTexture_RenderThread(RHICmdList, BackBuffer, CastingViewportRenderTexture_RenderThread);
        }
		else if (MirrorWindowMode == EMirrorWindowMode::Distorted)
#else
...
#endif
		{
			...
			if (MirrorTexture)
			{
				CustomPresent->CopyTexture_RenderThread(RHICmdList, BackBuffer, MirrorTexture);
			}
		}
		else if (MirrorWindowMode == EMirrorWindowMode::Undistorted)
		{
			...
			for (int i = 0; i < 2; ++i)
			{
				CustomPresent->CopyTexture_RenderThread(RHICmdList, BackBuffer, SrcTexture, destRect, Settings_RenderThread->EyeRenderViewport[i]);
			...
			MirrorWindowMode == EMirrorWindowMode::SingleEyeCroppedToFill)
		{
			...
			CustomPresent->CopyTexture_RenderThread(RHICmdList, BackBuffer, SrcTexture, DstViewRect, SrcViewRect, false, false);
		}
	}
...
void FOculusHMD::GetOrthoProjection(int32 RTWidth, int32 RTHeight, float OrthoDistance, FMatrix OrthoProjection[2]) const
{
	...
	for (int eyeIndex = 0; eyeIndex < 2; eyeIndex++)
	{
		...
		float widthDivider = Settings->Flags.bIsUsingDirectMultiview ? 1.0f : 2.0f;
...
bool FOculusHMD::AllocateRenderTargetTexture(uint32 Index, uint32 SizeX, uint32 SizeY, uint8 Format, uint32 NumMips, uint32 InTexFlags, uint32 InTargetableTextureFlags, FTexture2DRHIRef& OutTargetableTexture, FTexture2DRHIRef& OutShaderResourceTexture, uint32 NumSamples)
{
	...
	{
		InitializeEyeLayer_RenderThread();
...
IStereoLayers::FLayerDesc FOculusHMD::GetDebugCanvasLayerDesc(FTextureRHIRef Texture)
{
	...
	StereoLayerDesc.Transform = FTransform(FVector(70.0, 0, 0));
	StereoLayerDesc.CylinderHeight = 150.f;
	StereoLayerDesc.CylinderOverlayArc = 270.f;
	StereoLayerDesc.CylinderRadius = 150.f;
...
void FOculusHMD::InitializeEyeLayer_RenderThread()
{
	CheckInRenderThread();
...
//		UE_LOG(LogHMD, Log, TEXT("StartRHIFrame %u"), Frame_RenderThread->FrameNumber);
		...
		ExecuteOnRHIThread_DoNotWait([this, XSettings, XFrame, XLayers]()
		{
			if (XFrame.IsValid())
			{
				...
				{
//					UE_LOG(LogHMD, Log, TEXT("ovrp_BeginFrame3 %u"), Frame_RHIThread->FrameNumber);
...
//			UE_LOG(LogHMD, Log, TEXT("ovrp_EndFrame3 %u"), Frame_RHIThread->FrameNumber);
