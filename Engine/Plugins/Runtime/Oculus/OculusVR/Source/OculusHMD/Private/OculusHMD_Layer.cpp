...
void FLayer::SetDesc(const IStereoLayers::FLayerDesc& InDesc)
{
	...
	Desc = InDesc;
#if PLATFORM_ANDROID // poke a hole is un-necessary on PC due to depth buffer sharing and compositing
...
void FLayer::BuildPokeAHoleMesh(TArray<FVector>& Vertices, TArray<int32>& Triangles, TArray<FVector2D>& UV0)
{
	...
	else if (Desc.ShapeType == IStereoLayers::CylinderLayer)
	{
		...
		const int Sides = (ArcAngle * 180) / ( PI * 5); // one triangle every 10 degrees of cylinder for a good-cheap approximation
