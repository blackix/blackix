...
void FLayer::Initialize_RenderThread(FCustomPresent* CustomPresent, const FLayer* InLayer)
{
	...
	else
	{
		...
		if(bLayerCreated)
		{
			...
			TextureSetProxy = CustomPresent->CreateTextureSet_RenderThread(SizeX, SizeY, Format, NumMips, NumSamples, ArraySize, Textures);

			if(OvrpLayerDesc.Layout == ovrpLayout_Stereo)
			{
				RightTextureSetProxy = CustomPresent->CreateTextureSet_RenderThread(SizeX, SizeY, Format, NumMips, NumSamples, ArraySize, RightTextures);
			}
		}
		...
		case ovrpShape_Cylinder:
			{
				...
				OvrpLayerSubmit.Cylinder.ArcWidth = Desc.CylinderSize.X * Scale.x;
				OvrpLayerSubmit.Cylinder.Height = CylinderHeight * Scale.x;
				OvrpLayerSubmit.Cylinder.Radius = Desc.CylinderSize.Y * Scale.x;
			}
			break;
		}
