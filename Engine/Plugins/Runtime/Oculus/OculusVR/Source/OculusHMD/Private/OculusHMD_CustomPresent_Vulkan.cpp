...
FTextureSetProxyPtr FVulkanCustomPresent::CreateTextureSet_RenderThread(uint32 InSizeX, uint32 InSizeY, EPixelFormat InFormat, uint32 InNumMips, uint32 InNumSamples, uint32 InArraySize, const TArray<ovrpTextureHandle>& InTextures)
{
	CheckInRenderThread();
	...
	{
		RHITexture = nullptr;//DynamicRHI->RHICreateTexture2DFromVkImage(InFormat, InSizeX, InSizeY, (VkImage)InTextures[0], TexCreate_ShaderResource);
	}
