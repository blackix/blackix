...
void FOculusRiftHMD::RecordAnalytics()
{
	if (FEngineAnalytics::IsAvailable())
	{
		...
		EventAttributes.Add(FAnalyticsEventAttribute(TEXT("LateLatching"), Settings->Flags.bLateLatching));
		EventAttributes.Add(FAnalyticsEventAttribute(TEXT("LateLatchingOrientation"), Settings->Flags.bLateLatchingOrientation));
...
void FOculusRiftHMD::Startup()
{
	...
	if (IsPCPlatform(GMaxRHIShaderPlatform) && !IsOpenGLPlatform(GMaxRHIShaderPlatform))
	{
		...
		check(!LateLatching);
		LateLatching = new FLateLatchingD3D(this);
	}
#endif
#if defined(OVR_GL)
...
void FOculusRiftHMD::LoadFromIni()
{
	...
	if (GConfig->GetBool(OculusSettings, TEXT("bLateLatching"), v, GEngineIni))
	{
		Settings->Flags.bLateLatching = v;
	}
	if (GConfig->GetBool(OculusSettings, TEXT("bLateLatchingOrientation"), v, GEngineIni))
	{
		Settings->Flags.bLateLatchingOrientation = v;
	}
...
void FOculusRiftHMD::SaveToIni()
{
	...
	GConfig->SetBool(OculusSettings, TEXT("bLateLatching"), Settings->Flags.bLateLatching, GEngineIni);

	GConfig->SetBool(OculusSettings, TEXT("bLateLatchingOrientation"), Settings->Flags.bLateLatchingOrientation, GEngineIni);
