...
{
	check(TexWidth != 0 && TexHeight != 0);

	if (!ColorTextureSet || (ColorTextureSet->GetSizeX() != TexWidth || ColorTextureSet->GetSizeY() != TexHeight || ColorTextureSet->GetFormat() != Format))
	{
		bNeedReAllocateTextureSet = true;
	}
	...
	if (Hmd && bNeedReAllocateTextureSet)
	{
		...
		dsDesc.Width = TexWidth;
		dsDesc.Height = TexHeight;
		dsDesc.MipLevels = 1;
		dsDesc.ArraySize = 1;
		...
		if (!textureSet)
		{
			UE_LOG(LogHMD, Error, TEXT("Can't create swap texture set (size %d x %d)"), TexWidth, TexHeight);
			return nullptr;
		}
		bNeedReAllocateTextureSet = false;
		bNeedReAllocateMirrorTexture = true;
		UE_LOG(LogHMD, Log, TEXT("Allocated a new swap texture set (size %d x %d)"), TexWidth, TexHeight);
	...
	return ColorTextureSet->GetTexture2D();
}
