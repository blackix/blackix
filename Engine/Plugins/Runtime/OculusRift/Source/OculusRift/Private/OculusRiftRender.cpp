	...
	if (frame)
	{
		if (frame->Settings.Flags.bDrawGrid)
		{
			...
			for (int eye = 0; eye < 2; ++eye)
			{
				...
				lineStep = 48;
				OVR::Vector2f rendertargetNDC = OVR::FovPort(frame->Settings.EyeRenderDesc[eye].Fov).TanAngleToRendertargetNDC(OVR::Vector2f(0.0f));
		...
		if (IsStereoEnabled() && frame->Settings.Flags.bShowStats)
		{
			...
			if ((frame->Settings.SupportedHmdCaps & ovrHmdCap_DynamicPrediction) != 0)
			{
				...
				if (ovrHmd_GetFloatArray(Hmd, "DK2Latency", latencies, 3) == 3)
				{
					...
					FCStringAnsi::Snprintf(destStr, sizeof(destStr), "Latency, ren: %s tw: %s pp: %s",
