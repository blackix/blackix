...
void FLayerManager::PreSubmitUpdate_RenderThread(FRHICommandListImmediate& RHICmdList, const FHMDGameFrame* InCurrentFrame, bool ShowFlagsRendering)
{
	...
	for (uint32 i = 0; i < LayersToRender.Num() ; ++i)
	{
		...
		case FHMDLayerDesc::Quad:
			...
			bool reloadTex = LayerDesc.IsTextureChanged();
			...
			if (texPtr && texPtr->IsValidLowLevel() && texPtr->Resource && texPtr->Resource->TextureRHI) 
			{
				...
				const uint32 SizeX = texRHIPtr->GetTexture2D()->GetSizeX()+2;
				const uint32 SizeY = texRHIPtr->GetTexture2D()->GetSizeY()+2;
				...
				if (reloadTex && RenderLayer->TextureSet.IsValid()) 
				{
					if (texRHIPtr) 
					{
						pPresentBridge->CopyTexture_RenderThread(RHICmdList, RenderLayer->TextureSet->GetRHITexture2D(), texRHIPtr->GetTexture2D(), FIntRect(), FIntRect(), true);
					}
				}
...
void FGearVRCustomPresent::CopyTexture_RenderThread(FRHICommandListImmediate& RHICmdList, FTexture2DRHIParamRef DstTexture, FTexture2DRHIParamRef SrcTexture,
