...
bool FGearVRController::GetControllerOrientationAndPosition( const int32 ControllerIndex, const EControllerHand DeviceHand, FRotator& OutOrientation, FVector& OutPosition ) const
{
	...
	if (CurrentRemoteID != ovrDeviceIdType_Invalid)
	{
		...
		if (GearVR)
		{
			...
			if (OvrMobile.IsValid())
			{
				...
				{
					CurrentFrame = GearVR->GetCustomPresent_Internal()->GetRenderFrame();
				}
				else
				{
