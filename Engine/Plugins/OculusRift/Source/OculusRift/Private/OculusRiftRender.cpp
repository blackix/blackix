...
#ifdef OVR_DIRECT_RENDERING 
FRHIBridge* FOculusRiftHMD::GetActiveRHIBridge()
{
...
{
	FBridgeRHIRef bridge = GetActiveRHIBridge();
	if (bridge && RHIGetBridge() == bridge)
	{
		RHISetBridge(NULL);
#if defined(OVR_D3D_VERSION) && (OVR_D3D_VERSION == 11)
		if (pD3D11Bridge)
...
void FOculusRiftHMD::UpdateViewport(const FViewport& Viewport, const FRHIViewport* ViewportRHI)
{
	check(IsInGameThread());
	...
	GetActiveRHIBridgeImpl()->UpdateViewport(Viewport, (ViewportRHI) ? ViewportRHI : Viewport.GetViewportRHI().GetReference());
}
...
void FOculusRiftHMD::D3D11Bridge::Init(ID3D11Device* InD3DDevice, ID3D11DeviceContext* InD3DDeviceContext)
{
	check(IsInGameThread());
	...
	Cfg.D3D11.pDevice			= InD3DDevice;
	Cfg.D3D11.pDeviceContext	= InD3DDeviceContext;
...
void FOculusRiftHMD::D3D11Bridge::UpdateViewport(const FViewport& Viewport, const FRHIViewport* ViewportRHI)
{
	check(IsInGameThread());
	check(ViewportRHI);
...
void FOculusRiftHMD::D3D11Bridge::ReleaseBackBuffer()
{
	Cfg.D3D11.pBackBufferRT		= nullptr;
	Cfg.D3D11.pSwapChain		= nullptr;
...
void FOculusRiftHMD::OGLBridge::ReleaseBackBuffer()
{
	bNeedReinitRendererAPI = true;
...
void FOculusRiftHMD::OGLBridge::UpdateViewport(const FViewport& Viewport, const FRHIViewport* ViewportRHI)
{
	check(IsInGameThread());
	check(ViewportRHI);
...
bool FOculusRiftHMD::OGLBridge::FinishFrame(int SyncInterval)
{
	check(IsInRenderingThread());
